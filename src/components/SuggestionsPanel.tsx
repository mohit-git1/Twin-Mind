  ...
  return (
    <div className="flex flex-col h-full w-full relative bg-white">
      ...
      <div className="flex-none flex items-center justify-between px-5 py-4 border-b border-slate-100">
        <h2 className="text-sm font-bold tracking-tight text-[#0f2e4a]">Live Suggestions</h2>
        <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">{suggestions.length} batches</span>
      </div>

      {/* Controls / Countdown */}
      <div className="flex items-center justify-between px-5 py-3 flex-none border-b border-slate-100 bg-slate-50/50">
        <button
          onClick={fetchSuggestions}
          disabled={isGeneratingSuggestions || transcript.length === 0}
          className="flex items-center gap-2 px-3 py-1.5 text-[10px] font-bold uppercase tracking-wider text-slate-600 border border-slate-200 rounded-lg hover:bg-white hover:border-sky-300 hover:text-sky-600 transition-all disabled:opacity-40 shadow-sm"
        >
            ...
            <div className="flex items-center gap-2 text-slate-400">
              ...
              <div className="relative w-3.5 h-3.5 flex items-center justify-center">
                <svg className="w-3.5 h-3.5 transform -rotate-90" viewBox="0 0 24 24">
                  <circle className="text-slate-200" strokeWidth="4" stroke="currentColor" fill="transparent" r="10" cx="12" cy="12" />
                  <circle 
      ...
      {/* Scrollable Area */}
      <div className="flex-1 overflow-y-auto px-5 py-4 space-y-6 custom-scrollbar bg-slate-50/30" ref={scrollRef}>
        {isGeneratingSuggestions && suggestions.length === 0 && (
           <div className="p-5 border border-slate-200 rounded-2xl bg-white animate-pulse shadow-sm">
        ...
        {suggestions.length === 0 && !isGeneratingSuggestions ? (
          <div className="flex h-full items-center justify-center">
            <p className="text-slate-400 italic text-sm text-center">Waiting for context...</p>
          </div>
        ) : (
            ...
            <div key={batch.id} className="space-y-4">
              ...
              {index !== suggestions.length - 1 && (
                <div className="flex items-center justify-center py-4">
                    ...
                    <span className="mx-4 text-[9px] text-slate-300 font-bold uppercase tracking-widest">Older History</span>
                    <div className="h-px bg-slate-200 w-10"></div>
                </div>
              )}
            </div>
